...
def get_corruption_path(corruption_root, corruption, severity, filepath):
    ...
    return os.path.join("/workspace/data/nuscenes/samples", subfolder, filename)
...
def render_sample_data(
    ...
    """
    lidar_path = os.path.join(out_path, 'BEV')
    if not os.path.exists(lidar_path): os.makedirs(lidar_path)
    lidiar_render(sample_toekn, pred_data, out_path=os.path.join(lidar_path, sample_toekn))
    ...
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            # Init axes.
            # data = make_corruption(data, corruptions)
            # Show image.
        ...
        if out_path is not None:
            camera_path = os.path.join(out_path, cam)
            if not os.path.exists(camera_path): os.makedirs(camera_path)
            plt.savefig(os.path.join(camera_path, sample_toekn), bbox_inches='tight', pad_inches=0, dpi=200)
        if verbose:
            plt.show()
        plt.cla()
    plt.close()
...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load('/workspace/test_dirs/pts_bbox/results_nusc.json')
    # sample_token_list = list(bevformer_results['results'].keys())

    
    # for id in range(0, 1):
    # render_sample_data('3502d36d2c76428f9b79f2854e96b696', pred_data=bevformer_results, out_path='/workspace/vis',
    ...
    render_sample_data('3e8750f331d7499e9b5123e9eb70f2e2', pred_data=bevformer_results, out_path='/workspace/vis',
                        corruption_root='/nvme/konglingdong/data/sets/nuScenes-c',
                        corruption=corruption,
                        severity='easy')
